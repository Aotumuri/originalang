...
import {
  buildDirectoryRelativePath,
  getPartOfSpeechFileName,
  nowIsoString,
} from "./utils";
...
function buildPartOfSpeechFile(part: PartFileInfo): string {
...
function buildMetadata(snapshot: DictionarySnapshot): string {
  return [
    "# Dictionary Metadata",
    "Version: 1",
...
function buildAllWordsIndex(snapshot: DictionarySnapshot): string {
  const lines = ["# 全単語索引"];
...
function buildCategoriesFile(categories: ManagedEntity[]): string {
  const lines = ["# カテゴリ一覧"];
...
function buildCategoryIndex(snapshot: DictionarySnapshot): string {
  const lines = ["# カテゴリ別索引"];
...
function buildReadme(partFiles: PartFileInfo[]): string {
...
function buildPartFiles(snapshot: DictionarySnapshot): PartFileInfo[] {
...
export async function buildTextBundle(): Promise<{ relativePath: string; absolutePath: string }> {
  ...
  await writeTextFile(buildDirectoryRelativePath("categories.txt"), buildCategoriesFile(snapshot.categories), {
    baseDir: BaseDirectory.AppData,
  });
  await writeTextFile(buildDirectoryRelativePath("category-index.txt"), buildCategoryIndex(snapshot), {
    baseDir: BaseDirectory.AppData,
  });
